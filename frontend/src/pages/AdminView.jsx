...
import { 
  UserPlus, Users, ClipboardList, Settings, LogOut, 
  Trash2, Bell, Moon, Shield, Database, Eye, 
  Edit, Lock, Save, Activity 
} from "lucide-react";
...
function AdminView() {
  ...
  useEffect(() => {
    if (canEnter) {
      setActivityLog([
        { id: 1, user: "System", action: "Loaded physician directory", timestamp: new Date().toISOString() },
        { id: 2, user: user?.name || "Current User", action: "Admin login", timestamp: new Date().toISOString() }
      ]);
    }
  }, [canEnter, user]);
  ...
  const handleInviteSubmit = async (e) => {
    ...
    try {
      ...
      if (responseText.trim()) {
        ...
        alert(`Physician ${inviteFirstName} ${inviteLastName} added successfully`);
      } else {
  ...
  const handleDeletePhysician = async (id) => {
    ...
    try {
      ...
      // remove from state
      const deletedPhysician = physicians.find(physician => physician.id === id);
  ...
  const handlePermissionToggle = async (physicianId, permission) => {
    ...
    setPhysicians(physicians.map(p => p.id === physicianId ? { ...p, permissions: updatedPermissions } : p));
    
    const newActivityLog = {
      id: activityLog.length + 1,
      user: user?.name || "Admin",
      action: `${hasPermission ? 'Removed' : 'Added'} ${permission} permission for ${physician.name}`,
      timestamp: new Date().toISOString()
    };
      ...
      <div className="w-full mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div className="flex justify-between items-center mb-6">
          ...
          <div className="flex items-center">
            {user?.picture ? (
              <img src={user.picture} alt="Profile" className="h-8 w-8 sm:h-10 sm:w-10 rounded-full mr-2 sm:mr-3" />
            ) : (
            ...
            <span className="font-medium text-sm sm:text-base text-gray-700 mr-3">{user?.name || "User"}</span>
            <button className="text-gray-500 hover:text-red-600 transition-colors" title="Logout">
        ...
        <div className="bg-white rounded-lg shadow-md w-full">
          ...
          <div className="border-b border-gray-200 overflow-x-auto w-full">
            <nav className="flex w-full">
              <TabButton tab="invitations" icon={UserPlus} label="Add Physician" />
              <TabButton tab="staff" icon={Users} label="Physician Management" />
              <TabButton tab="activity" icon={ClipboardList} label="Activity Log" />
          ...
          <div className="p-4 sm:p-6 w-full">
            ...
            {activeTab === "invitations" && (
              <div className="w-full">
                ...
                <p className="text-gray-600 mb-6">Add a new physician to the system.</p>
                ...
                <form onSubmit={handleInviteSubmit} className="w-full">
                  ...
                  <div className="mb-4">
                    ...
                    <input
                      type="email" id="email" placeholder="physician@example.com"
                      className="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                  ...
                  <div className="mb-4">
                    ...
                    <select
                      id="role" defaultValue="physician" disabled
                      className="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                    >
                      <option value="physician">Physician</option>
                    </select>
                    <p className="text-xs text-gray-500 mt-1">Currently only physician role is supported</p>
                  </div>
            ...
            {activeTab === "staff" && (
              <div className="w-full">
                ...
                <p className="text-gray-600 mb-6">Manage physicians and their permissions.</p>
                
                {physicians.length === 0 ? (
                  <div className="text-center py-8 text-gray-500 flex flex-col items-center">
                    <Users className="h-12 w-12 mb-3 text-gray-400" />
                    <p>No physicians found. Add physicians using the Add Physician tab.</p>
                  </div>
                ) : (
                  <div className="overflow-x-auto w-full">
                    <table className="min-w-full divide-y divide-gray-200">
                      <thead className="bg-gray-50">
                        <tr>
                          <th className="px-3 py-3 sm:px-6 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Name</th>
                          <th className="px-3 py-3 sm:px-6 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Email</th>
                          <th className="px-3 py-3 sm:px-6 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Role</th>
                          <th className="px-3 py-3 sm:px-6 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Permissions</th>
                          <th className="px-3 py-3 sm:px-6 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Actions</th>
                        </tr>
                      </thead>
                        ...
                        {physicians.map((physician) => (
                          <tr key={physician.id}>
                            <td className="px-3 py-4 sm:px-6 whitespace-nowrap">
                              <div className="font-medium text-gray-900">{physician.name}</div>
                            </td>
                            <td className="px-3 py-4 sm:px-6 whitespace-nowrap">
                              <div className="text-gray-500">{physician.email}</div>
                            </td>
                            <td className="px-3 py-4 sm:px-6 whitespace-nowrap">
                            ...
                            <td className="px-3 py-4 sm:px-6 whitespace-normal text-sm text-gray-500">
                              <div className="flex flex-wrap gap-2">
                                {[
                                  { key: "view_patients", icon: Eye, label: "view patients" },
                                  { key: "edit_records", icon: Edit, label: "edit records" },
                                  { key: "admin_access", icon: Lock, label: "admin access" }
                                ].map(({ key, icon, label }) => (
                                  <label key={key} className="inline-flex items-center">
                                    <input
                                      type="checkbox"
                                      className="form-checkbox h-4 w-4 text-blue-600"
                                      checked={physician.permissions.includes(key)}
                                      onChange={() => handlePermissionToggle(physician.id, key)}
                                    />
                                    <span className="ml-2 text-xs sm:text-sm flex items-center">
                                      {React.createElement(icon, { className: "h-3 w-3 mr-1 text-gray-500" })}
                                      {label}
                                    </span>
                                  </label>
                                ))}
            ...
            {activeTab === "activity" && (
              <div className="w-full">
                ...
                <p className="text-gray-600 mb-6">View system activity and action history.</p>
                
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8 w-full">
                  {[
                    { bg: "bg-blue-50", icon: Users, color: "text-blue-600", count: physicians.length, label: "Active Physicians" },
                    { bg: "bg-green-50", icon: Shield, color: "text-green-600", count: admins.length, label: "Active Admins" },
                    { bg: "bg-purple-50", icon: Activity, color: "text-purple-600", count: activityLog.length, label: "Recent Actions" }
                  ].map((item, index) => (
                    <div key={index} className={`${item.bg} p-4 rounded-lg flex items-center`}>
                      {React.createElement(item.icon, { className: `h-8 w-8 ${item.color} mr-3` })}
                      <div>
                        <div className={`${item.color} text-xl font-bold`}>{item.count}</div>
                  ...
                  <table className="min-w-full divide-y divide-gray-200">
                    <thead className="bg-gray-50">
                      <tr>
                        ...
                        <th className="px-3 py-3 sm:px-6 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Timestamp</th>
                      </tr>
                    </thead>
                    <tbody className="bg-white divide-y divide-gray-200">
                      {activityLog.map((log) => (
                  ...
                  {[
                    {
                      ...
                      content: (
                        <label className="flex items-center cursor-pointer">
                          ...
                          <div className="ml-3 text-gray-700">Enable email notifications</div>
                        </label>
                      )
                    },
                    {
                    ...
                    },
                    {
                      ...
                      content: (
                        <div className="mb-4">
                          <label htmlFor="autoLogout" className="block text-sm font-medium text-gray-700 mb-1 flex items-center">
                          ...
                          >
                            {[15, 30, 60, 120].map(minutes => (
                              <option key={minutes} value={minutes}>{minutes === 120 ? "2 hours" : `${minutes} minutes`}</option>
                            ))}
                          </select>
                        </div>
                      )
                    },
                    {
                      ...
                      content: (
                        <div className="mb-4">
                          <label htmlFor="dataRetention" className="block text-sm font-medium text-gray-700 mb-1 flex items-center">
                    ...
                    <div key={index} className="border border-gray-200 rounded-md p-4">
                      <h3 className="font-medium text-gray-900 mb-2 flex items-center">
                        {React.createElement(section.icon, { className: "h-4 w-4 mr-2 text-blue-600" })}
                        {section.title}
                      </h3>
                      {section.content}
                    </div>
