...
import { FileText, Upload, Trash2, Eye, ArrowLeft, PlusCircle, File } from "lucide-react";

const LabResultsPage = () => {
  const [labRecords, setLabRecords] = useState([
    { id: 1, name: "Blood Work Results - Jan 2025.pdf", date: "Jan 25, 2025", size: "3.2 MB" },
    { id: 2, name: "MRI Scan Report.pdf", date: "Jan 10, 2025", size: "8.7 MB" },
    { id: 3, name: "Cholesterol Test Results.pdf", date: "Dec 15, 2024", size: "1.4 MB" }
  ]);
  ...
  const handleUpload = () => {
    ...
    const newRecord = {
      id: labRecords.length + 1,
      name: selectedFile.name,
      date: new Date().toLocaleDateString(),
      size: (selectedFile.size / (1024 * 1024)).toFixed(1) + " MB"
    };
    
    setLabRecords([...labRecords, newRecord]);
    setSelectedFile(null);
    setUploadingFile(false);
  };
  
  const handleDelete = (id) => {
    setLabRecords(labRecords.filter(record => record.id !== id));
  };
      ...
      <div className="w-full mx-auto px-4 sm:px-6 lg:px-8 py-6">
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-2xl sm:text-3xl font-bold text-gray-800 flex items-center">
            <FileText className="h-6 w-6 sm:h-8 sm:w-8 mr-2 text-blue-600" />
            Lab Results
          </h1>
          <Link 
            to="/patient" 
          ...
          <div className="p-4 sm:p-6">
            <div className="mb-6">
              <p className="text-gray-700 mb-4">
                View and manage your lab test results. You can upload new files, view existing ones, or delete records you no longer need.
              </p>
              
              {uploadingFile ? (
                <div className="bg-blue-50 border border-blue-200 rounded-md p-4 mb-4">
                  <h3 className="font-medium text-blue-700 mb-2">Upload Lab Result</h3>
                    ...
                    {labRecords.length === 0 && (
                      <tr>
                        <td colSpan="4" className="px-6 py-4 text-center text-sm text-gray-500">
                          No records found. Upload your first lab result.
                        </td>
                      </tr>
                    )}
                  </tbody>
